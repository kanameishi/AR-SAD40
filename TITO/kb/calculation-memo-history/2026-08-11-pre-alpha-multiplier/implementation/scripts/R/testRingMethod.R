...
# Coulomb-limited transfer reproduces both analytical endpoints and bounds an
# intermediate interface by alpha_delta * effective normal pressure.
FrictionEndpointLoads <- list(
  fullTraction = k0InterfaceFrictionLoad(
    effectiveVertical = 100,
    k0 = 0.5,
    porePressure = 20,
    interfaceFrictionCoefficient = 1
  ),
  normalOnly = k0InterfaceFrictionLoad(
    effectiveVertical = 100,
    k0 = 0.5,
    porePressure = 20,
    interfaceFrictionCoefficient = 0
  )
)
for (Interface in names(FrictionEndpointLoads)) {
  FrictionValues <- evaluateRingLoad(FrictionEndpointLoads[[Interface]], Theta)
  ...
  assertNear(
    FrictionValues$radialOutward,
    TensorValues$radialOutward,
    5e-14,
    paste("Coulomb radial endpoint", Interface)
  )
  assertNear(
    FrictionValues$tangentialPositive,
    TensorValues$tangentialPositive,
    5e-14,
    paste("Coulomb tangential endpoint", Interface)
  )
}
IntermediateCoefficient <- 0.2
IntermediateLoad <- k0InterfaceFrictionLoad(
  effectiveVertical = 100,
  k0 = 0.5,
  porePressure = 20,
  interfaceFrictionCoefficient = IntermediateCoefficient
)
IntermediateValues <- evaluateRingLoad(IntermediateLoad, Theta)
EffectiveNormal <- 75 + 25 * cos(2 * Theta)
assertTrue(
  all(
    abs(IntermediateValues$tangentialPositive) <=
      IntermediateCoefficient * EffectiveNormal + 1e-12
  ),
  "Coulomb tangential capacity"
)
...
assertTrue(
  ...
  "Coulomb intermediate direct response"
)
assertTrue(
  all(is.finite(as.matrix(IntermediateResponse$values[, c(
    "normalForce", "bendingMoment", "shearForce"
  )]))),
  "Coulomb intermediate finite resultants"
)
assertNear(
  ...
  5e-6,
  "Coulomb intermediate integration convergence"
)
assertError(
  function() {
    k0InterfaceFrictionLoad(
      effectiveVertical = 100,
      k0 = 0.5,
      interfaceFrictionCoefficient = 1.01
    )
  },
  "Coulomb coefficient upper bound",
  "must not exceed 1"
)
