...
# Simplified Coulomb transfer law for the soil-liner interface.
#
# The projected tangential traction is treated as the available demand.  The
# transmitted traction cannot exceed adhesion plus the effective normal
# contact pressure times alpha_delta = tan(delta).  Pore-water pressure is not
# included in the frictional capacity.
k0InterfaceFrictionLoad <- function(
  effectiveVertical,
  k0,
  porePressure = 0,
  horizontalIncrement = 0,
  ...
  if (interfaceFrictionCoefficient > 1) {
    stop(
      "interfaceFrictionCoefficient must not exceed 1 for this study.",
      call. = FALSE
    )
  }
  .assertFiniteScalar(interfaceAdhesion, "interfaceAdhesion", minimum = 0)
  ...
  transferredTangential <- function(theta) {
    Candidate <- candidateTangential(theta)
    Capacity <- interfaceAdhesion +
      interfaceFrictionCoefficient * pmax(0, effectiveNormal(theta))
    sign(Candidate) * pmin(abs(Candidate), Capacity)
  }
  ...
  newRingLoad(
    ...
    tangential = transferredTangential,
    label = paste0(
      "K0 stress field with Coulomb interface, tan(delta) = ",
      format(interfaceFrictionCoefficient, trim = TRUE)
    ),
    source = paste(
      "derived stress projection limited by Coulomb interface capacity;",
      "CANDE-2025 Eq. 4.3-9"
    ),
    representation = "coulombLimitedTangentialTransfer",
    metadata = list(
      evidenceLevel = "derived",
      effectiveVertical = effectiveVertical,
      effectiveHorizontal = EffectiveHorizontal,
      porePressure = porePressure,
      horizontalIncrement = horizontalIncrement,
      interfaceFrictionCoefficient = interfaceFrictionCoefficient,
      interfaceFrictionAngleDeg =
        atan(interfaceFrictionCoefficient) * 180 / pi,
      interfaceAdhesion = interfaceAdhesion,
      tangentialDemand = "biaxial stress projection",
      tangentialCapacity = "adhesion + effective normal pressure * tan(delta)"
    )
  )
}
