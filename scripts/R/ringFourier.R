...
combineRingSpectra <- function(spectra) {
  if (!is.list(spectra) || length(spectra) == 0L) {
    stop("spectra must be a non-empty list of ring spectra.", call. = FALSE)
  }
  Validated <- lapply(spectra, validateRingSpectrum)
  MaxMode <- max(vapply(Validated, function(Spectrum) {
    max(Spectrum$mode)
  }, numeric(1)))
  Combined <- newRingSpectrum(MaxMode)
  Coefficients <- names(Combined)[-1L]

  for (Spectrum in Validated) {
    Rows <- match(Spectrum$mode, Combined$mode)
    Combined[Rows, Coefficients] <- Combined[Rows, Coefficients] +
      Spectrum[, Coefficients, drop = FALSE]
  }

  attr(Combined, "components") <- lapply(spectra, function(Spectrum) {
    list(
      source = attr(Spectrum, "source"),
      representation = attr(Spectrum, "representation")
    )
  })
  if (any(vapply(Validated, function(Spectrum) {
    identical(attr(Spectrum, "momentSupported"), FALSE)
  }, logical(1)))) {
    attr(Combined, "momentSupported") <- FALSE
  }
  if (any(vapply(Validated, function(Spectrum) {
    identical(attr(Spectrum, "shearSupported"), FALSE)
  }, logical(1)))) {
    attr(Combined, "shearSupported") <- FALSE
  }
  for (ScaleName in c("fitRadialLoadScale", "fitTangentialLoadScale")) {
    FitLoadScales <- vapply(Validated, function(Spectrum) {
      FitLoadScale <- attr(Spectrum, ScaleName)
      if (is.null(FitLoadScale)) 0 else FitLoadScale
    }, numeric(1))
    if (any(FitLoadScales > 0)) {
      attr(Combined, ScaleName) <- sum(FitLoadScales)
    }
  }
  Combined
}

scaleRingSpectrum <- function(spectrum, factor) {
  Spectrum <- validateRingSpectrum(spectrum)
  assertFiniteScalar(factor, "factor")
  Spectrum[, -1L] <- Spectrum[, -1L, drop = FALSE] * factor
  Spectrum <- copyRingMetadata(spectrum, Spectrum)
  for (ScaleName in c("fitRadialLoadScale", "fitTangentialLoadScale")) {
    FitLoadScale <- attr(Spectrum, ScaleName)
    if (!is.null(FitLoadScale)) {
      attr(Spectrum, ScaleName) <- abs(factor) * FitLoadScale
    }
  }
  attr(Spectrum, "scaleFactor") <- factor
  Spectrum
}
...
summarizeRingResponse <- function(responseSpectrum, scanPoints = NULL) {
  Response <- validateRingResponseSpectrum(responseSpectrum)
  Active <- apply(Response[, -1L, drop = FALSE], 1L, function(Values) {
    any(is.finite(Values) & abs(Values) > 0)
  })
  MaxMode <- if (any(Active)) max(Response$mode[Active]) else 0L

  if (is.null(scanPoints)) {
    ScanPoints <- max(720L, 64L * max(MaxMode, 1L))
  } else {
    MinimumScanPoints <- max(16L, 16L * max(MaxMode, 1L))
    assertFiniteScalar(scanPoints, "scanPoints", lower = MinimumScanPoints)
    if (scanPoints != as.integer(scanPoints)) {
      stop("scanPoints must be an integer.", call. = FALSE)
    }
    ScanPoints <- as.integer(scanPoints)
  }

  Theta <- (0:(ScanPoints - 1L)) * 2 * pi / ScanPoints
  Values <- evaluateRingResponse(Response, Theta)
  Quantities <- c("normalForce", "bendingMoment", "shearForce")
  Labels <- c(normalForce = "N", bendingMoment = "M", shearForce = "Q")
  CoefficientNames <- list(
    normalForce = c("nCos", "nSin"),
    bendingMoment = c("mCos", "mSin"),
    shearForce = c("qCos", "qSin")
  )
  Supported <- c(
    normalForce = TRUE,
    bendingMoment = !identical(attr(Response, "momentSupported"), FALSE),
    shearForce = !identical(attr(Response, "shearSupported"), FALSE)
  )
  Rows <- vector("list", length(Quantities) * 3L)
  Row <- 1L
  Step <- 2 * pi / ScanPoints

  evaluateOne <- function(angle, quantity) {
    evaluateRingResponse(Response, angle)[[quantity]][1L]
  }

  for (Quantity in Quantities) {
    if (!Supported[[Quantity]]) {
      for (Statistic in c("minimum", "maximum", "absoluteMaximum")) {
        Rows[[Row]] <- data.frame(
          resultant = unname(Labels[[Quantity]]),
          statistic = Statistic,
          value = NA_real_,
          theta = NA_real_,
          thetaDeg = NA_real_,
    ...
    GridValues <- Values[[Quantity]]
    HarmonicRows <- Response$mode >= 1L
    Constant <- all(
      Response[HarmonicRows, CoefficientNames[[Quantity]], drop = FALSE] == 0
    )

    if (Constant) {
      MinAngle <- 0
      MaxAngle <- 0
      MinValue <- GridValues[1L]
      MaxValue <- GridValues[1L]
    } else {
      MinIndex <- which.min(GridValues)
      MaxIndex <- which.max(GridValues)
      MinCenter <- Theta[MinIndex]
      MaxCenter <- Theta[MaxIndex]
      Minimum <- optimize(
        function(angle) evaluateOne(angle %% (2 * pi), Quantity),
        interval = c(MinCenter - Step, MinCenter + Step),
        maximum = FALSE,
        tol = .Machine$double.eps^0.5
      )
      Maximum <- optimize(
        function(angle) evaluateOne(angle %% (2 * pi), Quantity),
        interval = c(MaxCenter - Step, MaxCenter + Step),
        maximum = TRUE,
        tol = .Machine$double.eps^0.5
      )
      MinAngle <- Minimum$minimum %% (2 * pi)
      MaxAngle <- Maximum$maximum %% (2 * pi)
      MinValue <- Minimum$objective
      MaxValue <- Maximum$objective
    }

    AbsIsMax <- abs(MaxValue) >= abs(MinValue)
    AbsAngle <- if (AbsIsMax) MaxAngle else MinAngle
    AbsValue <- if (AbsIsMax) MaxValue else MinValue
    Entries <- list(
      c("minimum", MinValue, MinAngle),
      c("maximum", MaxValue, MaxAngle),
      c("absoluteMaximum", AbsValue, AbsAngle)
    )
    ...
    for (Entry in Entries) {
      Rows[[Row]] <- data.frame(
        resultant = unname(Labels[[Quantity]]),
        statistic = Entry[1L],
        value = as.numeric(Entry[2L]),
        theta = as.numeric(Entry[3L]),
        thetaDeg = as.numeric(Entry[3L]) * 180 / pi,
        scanPoints = ScanPoints,
        stringsAsFactors = FALSE
      )
      Row <- Row + 1L
...
bakerDiametricLoadSpectrum <- function(
  halfAngleDeg,
  maxMode,
  loadPerPatch = 1,
  radius = 1,
  ...
  assertFiniteScalar(halfAngleDeg, "halfAngleDeg", lower = 0)
  assertFiniteScalar(maxMode, "maxMode", lower = 2)
  assertFiniteScalar(loadPerPatch, "loadPerPatch", lower = 0, strictLower = TRUE)
  assertFiniteScalar(radius, "radius", lower = 0, strictLower = TRUE)
  assertFiniteScalar(ringWidth, "ringWidth", lower = 0, strictLower = TRUE)
  if (halfAngleDeg >= 90) {
    stop("halfAngleDeg must be less than 90 degrees.", call. = FALSE)
  }
  if (maxMode != as.integer(maxMode)) {
    stop("maxMode must be an integer.", call. = FALSE)
  }
  ...
  Spectrum <- newRingSpectrum(as.integer(maxMode))
  Spectrum$radialCos[1L] <- -loadPerPatch / (pi * radius * ringWidth)
  Even <- Spectrum$mode >= 2L & Spectrum$mode %% 2L == 0L
  Mode <- Spectrum$mode[Even]
  HalfAngle <- halfAngleDeg * pi / 180
  Shape <- if (halfAngleDeg == 0) {
    rep(1, length(Mode))
  } else {
    sin(Mode * HalfAngle) / (Mode * HalfAngle)
  }
  Spectrum$radialCos[Even] <- -2 * loadPerPatch /
    (pi * radius * ringWidth) * Shape
  Spectrum
}
