...
caseDefinitions <- data.frame(
  case = c("tangential-alpha-1", "tangential-alpha-0"),
  prescription = c(
    "Componente tangencial: α = 1.00",
    "Componente tangencial: α = 0.00"
  ),
  tangentialMultiplier = c(1, 0),
...
caseLoads <- lapply(seq_len(nrow(caseDefinitions)), function(index) {
  k0TangentialMultiplierLoad(
    ...
    tangentialMultiplier = caseDefinitions$tangentialMultiplier[index]
  )
})
...
extrema <- do.call(rbind, lapply(seq_len(nrow(caseDefinitions)), function(index) {
  ...
  current$tangentialMultiplier <- caseDefinitions$tangentialMultiplier[index]
  current$stage <- "Estado biaxial uniforme"
  current$unit <- unname(quantityUnits[current$resultant])
  current$evidenceLevel <- "DE"
  current[, c(
    "case", "stage", "prescription", "tangentialMultiplier",
...
inputs <- data.frame(
  group = c(
    ...
    rep("Estado de tensiones y acciones", 7)
  ),
  magnitude = c(
    ...
    "Multiplicador de la componente tangencial"
  ),
  symbol = c(
    "D_i", "R", "perfil", "t_0", "t_b", "A_p", "I_p", "E_theta",
    "EA_theta", "EI_theta", "eta_s", "sigma'_v,A",
    "K_0", "Delta u_A", "sigma'_h,A", "p_m", "Delta sigma", "alpha"
  ),
  value = c(
  ...
  condition = c(
    ...
    "Resultado derivado", "Resultado derivado", "Resultado derivado",
    "Valores adoptados para el escenario de comprobación"
  ),
  stringsAsFactors = FALSE
)
