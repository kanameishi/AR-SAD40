...
descomposición modal analítica del gradiente lineal entre clave y solera.
...
P_r(\theta)=-p_n(\theta),
\qquad
P_t(\theta)=\lambda_t p_t^*(\theta),
\qquad 0\leq\lambda_t\leq1.
$$ {#eq-methodology-tangential-projection}

$\lambda_t=1$ representa la proyección tangencial completa y $\lambda_t=0$ una
...
La implementación combina ambas componentes para cada revestimiento y cada
límite de interfaz:

$$
\begin{aligned}
N_\theta^{H}(\theta)&=N_\theta^{SE}(\theta)+\Delta N_\theta(\theta),\\
M_\theta^{H}(\theta)&=M_\theta^{SE}(\theta)+\Delta M_\theta(\theta),\\
Q_\theta^{H}(\theta)&=Q_\theta^{SE}(\theta)+\Delta Q_\theta(\theta).
\end{aligned}
$$ {#eq-methodology-hybrid-resultants}

La componente $SE$ se recalcula con $C^*$ y $F^*$ propios de la chapa y de
cada alternativa de hormigón. La corrección de gradiente
conserva la misma acción geostática, pero utiliza el radio baricéntrico de la
sección correspondiente. Las tres resultantes de cada fila proceden de una
misma combinación de acciones y de una misma posición angular.
