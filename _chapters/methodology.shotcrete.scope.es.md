...
Esta metodología establece el procedimiento para analizar secciones
circulares de hormigón proyectado y comprobar una franja longitudinal sometida
a las resultantes circunferenciales $N_\theta$, $M_\theta$ y $Q_\theta$. Las
secciones autónomas se clasifican como hormigón simple u hormigón armado. El
estudio paramétrico incorpora además una sección compuesta explícita formada
por la chapa existente y una malla interior.

En las secciones autónomas no se transfieren solicitaciones desde la chapa
existente. Para cada combinación se calculan el radio, las razones $C^*$ y
$F^*$ y las distribuciones angulares de las tres resultantes. La carga externa
de Schwartz--Einstein aporta los modos $n=0,2$ y la corrección equilibrada del
gradiente aporta $n=1,3$. El procedimiento se ejecuta para deslizamiento libre
y sin deslizamiento. La proyección del estado biaxial uniforme, la integración
directa y Fourier se conservan como controles separados de carga prescrita.

La sección compuesta adopta compatibilidad total entre la chapa, el hormigón y
la armadura interior. Su centroide y sus rigideces se determinan con los tres
componentes y las resultantes se recalculan antes de construir el dominio
$P$--$M$. Esta hipótesis exige transferencia completa de corte en la interfaz
y se presenta separadamente de las secciones autónomas.

## Referencias normativas

La familia internacional adoptada es ACI:

- ACI CODE-318-25 proporciona las disposiciones de resistencia seccional,
  servicio, durabilidad y detallado. El Capítulo 14 contiene la comprobación
  adoptada para la rama de hormigón simple [@ACI31825]. Las combinaciones de
  acciones proceden de la envolvente EV--EH de AASHTO/CIRSOC utilizada para
  estructuras enterradas rígidas.
...
2. calcular su centroide y su rigidez circunferencial, incluyendo los
   componentes de la sección compuesta cuando corresponda;
...
revestimiento y una de las condiciones de ACI CODE-318-25, 14.1.2. La
aplicabilidad exige comprobar que la estructura puede representarse mediante
arcos bidimensionales y que dispone del apoyo requerido. Las razones de
utilización del Capítulo 14 corresponden a la comprobación seccional; la
clasificación y el apoyo conservan verificaciones independientes.

La rama de hormigón armado implementa el dominio $P$--$M$ mediante
compatibilidad, equilibrio y factores $\phi$ de ACI CODE-318-25. La cuantía
mínima total por dirección se comprueba separadamente mediante ACI 318.2-14,
6.1.3; la distribución simétrica entre caras es una configuración del estudio,
no una armadura adoptada. La sección compuesta emplea el mismo equilibrio
seccional con la chapa como capa exterior de acero y recalcula sus demandas.
Las demás comprobaciones de una cáscara delgada se establecen con el articulado
aplicable de ACI CODE-318.2-25.
